...
import eu.openaire.resourcesregistry.utils.RegexUtils;
...
@Component
public class LlmContentRequestClient implements LlmClient {

    private static final Logger logger = LoggerFactory.getLogger(LlmContentRequestClient.class);

    private final ChatModel chatModel;
    private final ContentMetadataRequestBuilder contentBuilder;
    private final ObjectMapper mapper = new ObjectMapper();

    public LlmContentRequestClient(ChatModel chatModel, ContentMetadataRequestBuilder contentBuilder) {
    ...
    public JsonNode generateMetadata(JsonNode template, Content content) {
        logger.debug("Request Token Count: {}", RegexUtils.countOccurrences(content.toString(), "(\\w+)"));
    ...
    public ArrayNode translate(ArrayNode content) {
        ...
        Translate the following JSON list to English and return it in the same format.
